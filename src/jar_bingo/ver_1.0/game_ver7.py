    ...
    for i in range(BOARD_SIZE):
        for j in range(BOARD_SIZE):
            ...
            pygame.draw.rect(screen, DARK_BLUE1,
                             ...
                             2)  # (i * CELL_SIZE, j * CELL_SIZE, CELL_SIZE, CELL_SIZE), 2)  # Red border
            text = font.render(string_parser(board[i][j][0]), True, BLACK)
            text_rect = text.get_rect(center=(BOARD_PADDING + i * CELL_SIZE + CELL_SIZE // 2,
                                              BOARD_PADDING + j * CELL_SIZE + CELL_SIZE // 2))  # (i * CELL_SIZE + CELL_SIZE // 2, j * CELL_SIZE + CELL_SIZE // 2))
...
def show_quiz_card(model, screen, font, preposition, choices, quiz_card_surface):
    ...
    pygame.draw.rect(screen, DARK_BLUE1, (QUIZ_CARD_PADDING, QUIZ_CARD_PADDING + 20, QUIZ_CARD_WIDTH, QUIZ_CARD_HEIGHT),
                     1)  # (SCREEN_WIDTH // 2 - QUIZ_CARD_WIDTH // 2, SCREEN_HEIGHT // 2 - QUIZ_CARD_HEIGHT // 2, QUIZ_CARD_WIDTH, QUIZ_CARD_HEIGHT)
    # question and choices
    question_text = font.render(string_parser(quiz_question), True, BLACK)
    question_rect = question_text.get_rect(center=(
        QUIZ_CARD_WIDTH // 2 + 100,
        120 + (CHOICE_RECT_HEIGHT + CHOICE_RECT_PADDING)))  # center=(QUIZ_CARD_WIDTH // 2, 50))
    screen.blit(question_text, question_rect)
    # blit card choices
    choice_rects = []
    for i, choice in enumerate(quiz_choices):
        choice_text = font.render(string_parser(choice), True, BLACK)
        choices_size = pygame.Rect(QUIZ_CARD_PADDING + 100, QUIZ_CARD_HEIGHT + (len(choices) * i),
        ...
        for event in pygame.event.get():
            ...
            # user clicked on a cell in the board
                ...
                if clicked_cell and all(i != clicked_cell for i in clicked_cells):
                    ...
                    quiz_card_shown, choice_rects = show_quiz_card(model, screen, font, preposition, quiz_choices,
                                                                   quiz_card_surface)
                    ...
                    if choice_rect.collidepoint(event.pos):
                        ...
                        if selected_choice == preposition:
                            ...
                            board[correct_cell[0]][correct_cell[1]] = (
                                board[correct_cell[0]][correct_cell[1]][0], GREEN)
                            ...
                            board[incorrect_cell[0]][incorrect_cell[1]] = (
                                board[incorrect_cell[0]][incorrect_cell[1]][0], RED)
