...
def initialize_game():
    ...
    return (model, screen, font, board, clicked_cells, quiz_card_shown, quiz_card_surface, background_image, jellyfish_tiles, game_over)
...
def draw_board(board, screen, background_image):
    ...
    for i in range(BOARD_SIZE):
        ...
        # No need to draw text as prepositions are replaced with images
            pygame.draw.rect(screen, color_cell, (BOARD_PADDING + i * CELL_SIZE, BOARD_PADDING + j * CELL_SIZE, CELL_SIZE, CELL_SIZE)) #(i * CELL_SIZE, j * CELL_SIZE, CELL_SIZE, CELL_SIZE))  # cell color fill
            pygame.draw.rect(screen, DARK_BLUE1, (BOARD_PADDING + i * CELL_SIZE, BOARD_PADDING + j * CELL_SIZE, CELL_SIZE, CELL_SIZE), 2) #(i * CELL_SIZE, j * CELL_SIZE, CELL_SIZE, CELL_SIZE), 2)  # border
...
def show_quiz_card(model, screen, font, preposition, choices, quiz_card_surface):
    ...
    pygame.draw.rect(screen, DARK_BLUE1, (QUIZ_CARD_PADDING, QUIZ_CARD_PADDING+20, QUIZ_CARD_WIDTH, QUIZ_CARD_HEIGHT), 3) 
    #question and choices
    question_text = font.render(string_parser(quiz_question), True, BLACK)
    question_rect = question_text.get_rect(center=(QUIZ_CARD_WIDTH // 2 + 100, 120 + (CHOICE_RECT_HEIGHT + CHOICE_RECT_PADDING)))
